...
local_run_id = f"cloud-helper-failed-smoke-{int(time.time())}"
...
cfg = {
    ...
    "seed": 42,
    "paths": {
        "root": tempfile.mkdtemp(prefix="saltai-cloud-helper-failed-smoke-"),
    },
}
...
client, cloud_run, event_bus = create_cloud_event_bus(
    ...
    config=cfg,
    manifest={
        "source": "saltai-cloud-helper-failed-smoke",
        "local_run_id": local_run_id,
    },
    tags=["smoke", "cloud", "helper", "failed"],
    raise_on_error=True,
)
...
def body(ctx):
    ctx.io.log_metric("loss", 0.999, step=1, epoch=1, split="train")
    raise RuntimeError("intentional cloud helper failed smoke")


try:
    runner.run(cfg, body=body)
except RuntimeError as exc:
    print("\nlocal runner failed as expected:")
    print(repr(exc))
